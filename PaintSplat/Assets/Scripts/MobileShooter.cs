...
public class MobileShooter : MonoBehaviour {
    ...
    public void ShootBall(Vector3 velocity)
    {
        ...
        // TODO-2.c PhotonNetwork.Instantiate to shoot a ball! DONE
        ...
        //   upon hitting a wall.
        PhotonView photon_view = PhotonView.Get(PhotonNetwork.Instantiate("ball",
                                                                          GameObject.Find("ARCamera").transform.position,
                                                                          Quaternion.identity,
                                                                          0));
        photon_view.RPC("RPCInitialize",
                        PhotonTargets.All,
                        velocity,
                        color_v);
    ...
    // change speed and velocity based on player's swipe
    public void ShootBallForward(Vector3 vel)
    {
        Vector3 direction = targetBehavior.GetPhoneForward();
        direction.x += (vel.x / 1.7f);
        ShootBall(ballSpeedFixed * direction * vel.y);
    }
